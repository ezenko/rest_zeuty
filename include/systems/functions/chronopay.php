<?php
/**
* Functions for chronopay payment system (send payment request to the payment system,
...
if (isset($dopayment_flag) && $dopayment_flag == 1) {
	...
	$data = array(
		...
		"amount"         => $amount,
		"order_id"       => $id_trunzaction."_".$id_service,
		"notify_url"    => $config["server"].$config["site_root"]."/include/payment_request.php?sel=chronopay",
		"decline_url" => $config["server"].$config["site_root"]."/account.php?sel=cancel_chronopay&order_id=".$id_trunzaction."_".$id_service,
...
function getBillingData($name, $lang) {
	global $dbconn;
		$rs = $dbconn->Execute("Select p.name, p.used, bs.seller_id from
								".BILLING_PAYSYSTEMS_TABLE." p INNER JOIN ".BILLING_SYS_.$name." bs ON p.name = bs.name and p.name='".$name."'");
		$data["use"] = $rs->fields[1];
		$data["value"] = $rs->fields[2];
		if ($data["use"]) $checked = "checked"; else $checked="";
		$data["table_options"] =
		        "<tr>".
                        "<td>".$lang['pays']['chronopay_seller_id'].":&nbsp;</td>".
...
function getPaymentValue($valuename,$__POST) {
	switch ($valuename) {
		case "count":	return $__POST["total"];
				break;
		case "curency":	return $__POST["currency"];
				break;
		case "date":	return date("Y-m-d H:i:s", strtotime($__POST["date"]) );
				break;
		case "status":	return ($__POST["transaction_type"] != "decline")?1:0;
				break;
		case "id_req":
				$arr = explode("_",$__POST["csl"]);
				return $arr[0];
				break;
		case "id_service":
				$arr = explode("_",$__POST["csl"]);
