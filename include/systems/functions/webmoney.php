<?php
/**
* Functions for webmoney payment system (send payment request to the payment system,
...
* @author $Author: irina $
* @version $Revision: 1.2 $ $Date: 2008/10/15 10:43:41 $
**/
...
function getPaymentValue($valuename,$__POST) {
	...
	switch ($valuename) {
		case "count":	return $__POST[$PaySystem->_arrayField['amount']];
				break;
		case "curency":	return $currency;
				break;
		case "date":	return date("Y-m-d H:i:s");
				break;
		case "status":
					return ($PaySystem->checkPayment())?1:0;
				break;
		case "id_req":
				$arr = explode("_",$__POST[$PaySystem->_arrayField['order_id']]);
				return $arr[0];
				break;
		case "id_service":
				$arr = explode("_",$__POST[$PaySystem->_arrayField['order_id']]);
