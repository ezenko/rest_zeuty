<?php
/**
* Functions for PayPal payment system (send payment request to the payment system,
...
if (isset($dopayment_flag) && $dopayment_flag == 1) {
    ...
    $data = array(
	    ...
	    "test_mode"     => "0",
	    "return_method" => "2",
	    "return_url"    => $config["server"].$config["site_root"]."/account.php",
	    "notify_url"    => $config["server"].$config["site_root"]."/include/payment_request.php?sel=$billing_system",
	    "cancel_return" => $config["server"].$config["site_root"]."/account.php",
	    "type"          => "_xclick",
...
function getPaymentValue($valuename,$__POST) {
	switch ($valuename) {
		case "count":	return $__POST["mc_gross"];
				break;
		case "curency":	return $__POST["mc_currency"];
				break;
		case "date":	return date("Y-m-d H:i:s", strtotime($__POST["payment_date"]) );
				break;
		case "status":	return ($__POST["payment_status"] == "Pending"||$__POST["payment_status"] == "Completed")?1:0;
				break;
		case "id_req":
				$arr = explode("_",$__POST["custom"]);
				return $arr[0];
				break;
		case "id_service":
				$arr = explode("_",$__POST["custom"]);
				return $arr[1];
				break;
		case "quantity":return $__POST["quantity"];
				break;
	}
}
?>