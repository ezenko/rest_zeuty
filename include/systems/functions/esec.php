<?php
/**
* Functions for esec payment system (send payment request to the payment system,
...
if (isset($dopayment_flag) && $dopayment_flag == 1) {
    ...
    $data = array(
		...
		"return_url"    => $config["server"].$config["site_root"]."/include/payment_request.php?sel=$billing_system&amount=<<payment_amount>>&payment_reference=<<payment_reference>>"
    );
...
function getPaymentValue($valuename,$__POST) {
	switch ($valuename) {
		...
		case "status":	return 1;
				break;
		case "id_req":
				$arr = explode("_",$__POST["payment_reference"]);
				return $arr[0];
				break;
		case "id_service":
				$arr = explode("_",$__POST["payment_reference"]);
