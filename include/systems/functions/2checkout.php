<?php
/**
* Functions for 2checkout payment system (send payment request to the payment system,
...
function getPaymentValue($valuename,$__POST) {
	switch ($valuename) {
		...
		case "date":		return date("Y-m-d H:i:s");
				break;
		case "status":		return ($_POST["credit_card_processed"] == "Y")?1:0;
				break;
		case "id_req":
				$arr = explode("_",$__POST["cart_order_id"]);
				return $arr[0];
				break;
		case "id_service":
				$arr = explode("_",$__POST["cart_order_id"]);
