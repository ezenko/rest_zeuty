...
function jsLoad(value, result_id, id_ad, upload_type, comment, user_upload_photo, choise, id_file, id_str_file, id_img_file) {
    ...
    req.open(null, 'rentals.php?sel=js_upload&id_ad='+id_ad+'&upload_type='+ upload_type +'&upload_comment='+comment, true);
    req.send( { q: value } );
}
{/literal}
</script>
...
<TR><TD valign="top">
	...
	<input name="id_spr_type" type="hidden" value="{$id_spr_type}">
	<input type="hidden" name="spr_realty_type[{$realty_type[0].num}]" value="{$realty_type[0].id}">
	<input type="hidden" name="realty_type[{$realty_type[0].num}][]" value="{$id_spr_type}">
	<table cellpadding="0" cellspacing="0" border="0" width="100%">
		<tr valign="top">
			<td class="header"><b>{if $id_ad}{$lang.headers.rental_ad_edit}{else}{$lang.headers.rental_ad_new}{/if}</b></td>
		</tr>
		{if !$mhi_services && !$id_ad && $use_listing_completion_bonus}
		<tr>
			<td><hr></td>
		</tr>
		<tr>
		...
		<tr>
			<td style="padding-left: 12px;">{$lang.content.your_ads_cnt} <b>{$user_ads_cnt}</b> {$lang.content.listings}.<br><br>{$lang.content.private_ads_limit_intro} - <b>{$private_person_ads_limit}</b> {$lang.content.private_ads_limit_message}<br><a href="{$server}{$site_root}/account.php">{$lang.content.change_status}</a></td>
		</tr>
		{else}
		<tr valign="top">
			<td class="subheader"><b>{$lang.content.page_header_step_1}</b></td>
		</tr>
		{/if}
	</table>
{if	!($private_person_ads_limit && $user_ads_cnt >= $private_person_ads_limit)}
			...
			<table cellpadding="5" cellspacing="0" border="0" width="100%">			
				<tr>
					...
					{if !$mhi_ad_buy}
						...
						<input type="radio" name="choise" value="3" onclick="Hide(3);" {if $choise==3 || ($pay_sell_lease && $choise != 1)}checked{/if}>&nbsp;{$lang.content.buy_realty}&nbsp;&nbsp;
						{/if}
					{/if}
					{if !$mhi_ad_sell}
						{if !$choise || $choise==4 || $choise==2 || !$id_ad}
						
							<input type="radio" name="choise" value="4" onclick="Show(4);" {if ($choise==4 && !$pay_sell_lease) || (!$pay_sell_lease && !$choise)}checked{/if} {if $pay_sell_lease && !$id_ad}disabled{/if}>&nbsp;{$lang.content.sell_realty}&nbsp;&nbsp;
						{/if}
					{/if}
					{if !$mhi_ad_rent}
						{if !$choise || $choise==3 || $choise==1 || !$id_ad}
					...
					{if !$mhi_ad_lease}
						{if !$choise || $choise==4 || $choise==2 || !$id_ad}
							<input type="radio" name="choise" value="2" onclick="Show(2);" {if $choise==2 && !$pay_sell_lease}checked{/if} {if $pay_sell_lease && !$id_ad}disabled{/if}>&nbsp;{$lang.content.have_room}
						{/if}
					{/if}	
					{/strip}
					</td>
				</tr>
				{if !$mhi_services && $pay_sell_lease}
				<tr>
					<td style="padding-bottom: 10px;">{$lang.content.sell_lease_need_pay} <a href="services.php?sel=sell_lease">{$lang.content.go_to_sell_lease}</a></td>
				</tr>
				{/if}				
				<tr>
					<td>					
						...
						<tr><td height="27" width="100">{$lang.content.select_country}&nbsp;:<font class="error">&nbsp;*</font></td>
							<td><div id="country_div" name="country_div">
									<select name="country" onchange="javascript: {literal} SelectRegion('rnte', this.value, document.getElementById('region_div'), document.getElementById('city_div'),'{/literal}{$lang.default_select.rnte_load_region}{literal}', '{/literal}{$lang.default_select.rnte_city}{literal}'); {/literal}" class="location">
										<option value="">{$lang.default_select.rnte_country}</option>
					...
					<td>					
						...
						<td><div id="region_div" name="region_div">
								<select name="region" onchange="javascript: {literal} SelectCity('rnte', this.value, document.getElementById('city_div'), '{/literal}{$lang.default_select.rnte_load_city}{literal}');{/literal}" class="location">
									<option value="">{$lang.default_select.rnte_region}</option>
					...
					<td>					
						...
						<td><div id="city_div" name="city_div">
								<select name="city" class="location">
									<option value="">{$lang.default_select.rnte_city}</option>
				...
				<tr>
					<td>
					<div id="subway_div" name="subway_div">					
					<table cellpadding="0" cellspacing="0" border="0" height="25">
						<tr>
							<td height="27" width="100">{$lang.default_select.select_subway}&nbsp;:</td>
							<td>
								<select name="subway" id="subway" style="width: 150px;">
									<option value="">{$lang.default_select.rnte_subway}</option>
				...
				<tr>
					<td width="100%">
					<div id="zip_code_div_1" {if ($choise eq 1 || $choise eq 3) || !($choise)} style="display: none;" {/if}>
					...
					<div id="street_div_1" {if ($choise eq 1 || $choise eq 3) || !($choise)} style="display: none;" {/if}>
						<table cellpadding="0" cellspacing="0" width="100%" border="0">
						<tr>
					...
					<div id="adress_div_1" {if ($choise eq 1 || $choise eq 3) || !($choise)} style="display: none;" {/if}>
						<table cellpadding="0" cellspacing="0" width="100%" border="0">
						<tr>
				...
				<tr>
					<td height="27"><input type="button" value="{$lang.buttons.save_1}" class="btn_small" onclick="javascript: document.step_1.action='{$form.next_link}'; document.step_1.submit();"></td>
				</tr>
			</table>
			</td>
		</tr>
		...
		<tr>
			...
			{section name=f loop=$realty_type}
				{if $realty_type[f].visible_in ne 3}<!--visibility checking-->
					<tr>
						...
						{/if}
						<div class="error_div" id="realty_type_error" style="display: none;">{$lang.errors.incorrect_field}</div>
						</td>
					</tr>
				{/if}
			{/section}
			...
			<tr>
				...
				<td align="left">
					...
					<tr>
						...
						<td>&nbsp;{$sq_meters}</td>
						<td align="left"><div class="error_div" id="min_live_square_error" style="display: none;">{$lang.errors.incorrect_field}</div>
			...
			<tr>
				...
				<td><input type="text" class="str" value="{$data.floor_num}" name="floor_num" id="floor_num" style="width: 50px;"'>&nbsp;{$lang.content.of_floors}
					<span class="error_div" name="floor_num_error" id="floor_num_error" style="display: none;">{$lang.errors.incorrect_field}</span>
				</td>
			</tr>
			<!-- /floor -->
			<!-- subway_min -->
			<tr>
				...
				<td class="text_small" align="left"><input type="text" class="str" name="subway_min" id="subway_min" size="10" value="{$data.subway_min}"'>&nbsp;{$lang.content.minutes}
					<span class="error_div" id="subway_min_error" style="display: none;">{$lang.errors.incorrect_field}</span>
				</td>
			</tr>
			<!-- /subway_min -->
			<!-- info -->
			...
			<tr>
				<td colspan="2" align="left">
					<input type="button" value="{$lang.buttons.save_1}" id="next_btn" class="btn_small" onclick="{literal}javascript: if (CheckStep('step_3')) {document.step_3.action='{/literal}{$form.next_link}{literal}'; document.step_3.submit();}{/literal}">
				</td>
			</tr>
			...
			<tr>
				...
				<td align="left">
					...
					<tr>
						...
						<td>
						<span name="min_payment_error" id="min_payment_error" style="display: none;" class="error_div">{$lang.errors.incorrect_field}</span>
						</td>
					</tr>
					</table>
					<!-- bidding possibility -->
			...
			<tr>
				...
				<td><input type="text" class="str" value="{$data.min_year_build}" name="min_year_build" id="min_year_build" style="width: 50px;">
					<span class="error" id="min_year_build_error" style="display: none;">&nbsp;&nbsp;{$lang.errors.incorrect_field}<br></span>
				</td>
			</tr>
			<!-- /year build -->
			<!-- description -->
			...
			<tr>
				...
				<td class="text_small" align="left"><input type="text" class="str" name="min_live_square" id="min_live_square" size="10" value="{$data.min_live_square}">&nbsp;{$sq_meters}
					<span class="error" id="min_live_square_error" style="display: none;">&nbsp;&nbsp;{$lang.errors.incorrect_field}</span>
				</td>
			</tr>
			<tr>
				<td width="110" valign="top" style="padding-top: 10px;">{$lang.content.total_square}:&nbsp;</td>
				<td class="text_small" align="left"><input type="text" class="str" name="min_total_square" id="min_total_square" size="10" value="{$data.min_total_square}">&nbsp;{$sq_meters}
					<span class="error" id="min_total_square_error" style="display: none;">&nbsp;&nbsp;{$lang.errors.incorrect_field}</span>
				</td>
			</tr>
			<tr>
				<td width="110" valign="top" style="padding-top: 10px;">{$lang.content.land_square}:&nbsp;</td>
				<td class="text_small" align="left"><input type="text" class="str" name="min_land_square" id="min_land_square" size="10" value="{$data.min_land_square}">&nbsp;{$sq_meters}
					<span class="error" id="min_land_square_error" style="display: none;">&nbsp;&nbsp;{$lang.errors.incorrect_field}</span>
				</td>
			</tr>
			<!-- /square -->
			<!-- floor -->
			<tr>
				...
				<td class="text_small" align="left"><input type="text" class="str" value="{$data.min_floor}" name="min_floor" id="min_floor" style="width: 50px;">&nbsp;{$lang.content.floor}
					<span class="error" id="min_floor_error" style="display: none;">&nbsp;&nbsp;{$lang.errors.incorrect_field}<br></span>
				</td>
			</tr>
			<tr>
				<td width="110" valign="top" style="padding-top: 10px;">{$lang.content.total_floor_num}:&nbsp;</td>
				<td><input type="text" class="str" value="{$data.floor_num}" name="floor_num" id="floor_num" style="width: 50px;">&nbsp;{$lang.content.of_floors}
					<span class="error" id="floor_num_error" style="display: none;">&nbsp;&nbsp;{$lang.errors.incorrect_field}</span>
				</td>
			</tr>
			<!-- /floor -->
			<!-- subway_min -->
			<tr>
				<td width="110" valign="top" style="padding-top: 10px;">{$lang.content.subway_min}:&nbsp;</td>
				<td class="text_small" align="left"><input type="text" class="str" name="subway_min" id="subway_min" size="10" value="{$data.subway_min}">&nbsp;{$lang.content.minutes}
					<span class="error" id="subway_min_error" style="display: none;">&nbsp;&nbsp;{$lang.errors.incorrect_field}</span>
				</td>
			</tr>
			<!-- /subway_min -->
			<!-- info -->
			...
			<tr>
				<td colspan="2" align="left">
					<input type="button" value="{$lang.buttons.save_1}" class="btn_small" onclick="{literal}javascript: if (CheckStep('step_3')) { document.step_3.action='{/literal}{$form.next_link}{literal}'; document.step_3.submit(); }{/literal}"></td>
				</td>
			</tr>
			{/if}
			</table>
...
<TR><TD>
	...
	<table cellpadding="0" cellspacing="0" border="0" width="100%">
		...
		<tr valign="top">
			<td class="subheader"><b>{if ($choise eq '1' || $choise eq '3')}{$lang.content.page_header_step_4}{else}{$lang.content.page_header_step_4_have}{/if}</b></td>
		</tr>
	</table>
	<table cellpadding="0" cellspacing="0" border="0" width="100%">
		<tr>
				...
				{if ($choise eq '2' || $choise eq '4')}
					...
					<tr>
						<td colspan="2">
							<table cellpadding="0" cellspacing="0">
								<tr>
									<td><input type="button" class="btn_small" value="{$lang.buttons.upload}" onclick=" document.getElementById('result_photo').innerHTML='<img src=\'{$site_root}{$template_root}{$template_images_root}/indicator.gif\' style=\'height:16px;\'>'; jsLoad(this.form.photo, 'result_photo','{$id_ad}', 'f', document.getElementById('upload_comment').value, 'user_upload_photo', '{$choise}', 'photo', 'file_text_photo', 'file_img_photo');"></td>
									<td style="padding-left:10px;">
...
<input name="id_ad" type="hidden" value="{$id_ad}">
	<table cellpadding="0" cellspacing="0" border="0" width="100%">
		...
		<tr valign="top">
			<td class="subheader"><b>{if ($choise eq '1' || $choise eq '3')}{$lang.content.page_header_step_5}{else}{$lang.content.page_header_step_5_have} ({if $choise eq 2}{$lang.content.about_leaser}{else if $choise eq 4}{$lang.content.about_buyer}{/if}){/if}</b></td>
		</tr>
	</table>
	<table cellpadding="0" cellspacing="0" border="0" width="100%">
		<tr>
						...
						<table cellpadding="0" cellspacing="0" border="0" class="upload_file">
							...
							<tr>
								<td><div class="fileinputs">
										<input type="file" name="photo" class="file" onmouseout="document.getElementById('file_text').value = this.value;" onkeyup="document.getElementById('file_text').value = this.value;"  />
										<div class="fakefile">
											<table cellpadding="0" cellspacing="0">
											<tr>
												<td>
							...
							<tr>
								<td><textarea name="upload_comment" cols="40" rows="5"></textarea></td>
							</tr>
							<tr>
			...
			<!-- have/sell  -->
				<tr>
					...
					<td>
						...
						<span name="age_error_div" class="error_div" id="age_error_div" style="display: none;">&nbsp;&nbsp;{$lang.errors.incorrect_field}</span>
					</td>
				</tr>
				...
				<tr>
					<td colspan="2" align="left">
						<input type="button" id="next_btn" value="{$lang.buttons.save_1}" class="btn_small" onclick="javascript: document.step_5.action='{$form.next_link}'; document.step_5.submit();">
					</td>
				</tr>
			{/if}
			</table>
...
<TR><TD>
	...
	<table cellpadding="0" cellspacing="0" border="0" width="100%">
		...
		<tr valign="top">
			<td class="subheader"><b>{if ($choise eq '1' || $choise eq '1')}{$lang.content.page_header_step_6}{else}{$lang.content.page_header_step_6_have}{/if}</b></td>
		</tr>
	</table>
	<table cellpadding="0" cellspacing="0" border="0" width="100%">
		<tr>
			...
			<table cellpadding="5" cellspacing="0" border="0" width="100%">
				<tr>
					<td width="110" valign="top" style="padding-top: 10px;">{$lang.content.comments}:&nbsp;</td>
				</tr>
				<tr>	
					<td align="left"><textarea name="comments" style="width: 200px; height: 75px;">{$comments}</textarea></td>
				</tr>
				<tr>
					<td align="left" colspan="2">
						<input type="button" value="{$lang.buttons.save_1}" class="btn_small" onclick="javascript: document.step_6.action='{$form.next_link}'; document.step_6.submit();">
					</td>
				</tr>
			</table>
			</td>
...
<TR><TD>
	...
	<table cellpadding="0" cellspacing="0" border="0" width="100%">
		<tr valign="top">
			<td class="header"><b>{$lang.headers.rental_ad_edit}&nbsp;|&nbsp;
			{if $choise eq '2'}{$lang.content.have_room_big}
			{elseif $choise eq '4'}{$lang.content.sell_realty_big}
			{/if}</b></td>
		</tr>
		<tr>
			<td class="subheader"><b>{$lang.content.planirovka_header}</b></td>
		</tr>
	</table>
	<table cellpadding="0" cellspacing="0" border="0" width="100%">
		<tr>
			<td width="10" valign="top">&nbsp;</td>
			<td valign="top">{if $error}
				<table cellpadding="0" cellspacing="0">
				<tr>
					<td>&nbsp;</td>
				</tr>
				...
				<tr>
					...
					<tr>
						<td colspan="2">
							<table cellpadding="0" cellspacing="0">
								<tr>
									<td><input type="button" class="btn_small" value="{$lang.buttons.upload}" onclick=" document.getElementById('result_plan').innerHTML='<img src=\'{$site_root}{$template_root}{$template_images_root}/indicator.gif\' style=\'height:16px;\'>'; jsLoad(this.form.plan, 'result_plan','{$id_ad}', 'plan', document.getElementById('upload_comment').value, 'user_upload_plan', '{$choise}', 'plan', 'file_text_plan', 'file_img_plan');"></td>
									<td style="padding-left:10px;">
...
<TR><TD>
	...
	<table cellpadding="0" cellspacing="0" border="0" width="100%">
		...
		<tr valign="top">
			<td class="subheader"><b>{if ($choise eq '1' || $choise eq '1')}{$lang.content.page_header_step_8}{else}{$lang.content.page_header_step_8_have}{/if}</b></td>
		</tr>
	</table>
	<table cellpadding="0" cellspacing="0" border="0" width="100%">
		<tr>
			...
			<table cellpadding="5" cellspacing="0" border="0" width="100%">
				<tr>
					<td width="110" valign="top" style="padding-top: 10px;">{$lang.content.headline}:&nbsp;</td>
				</tr>
				<tr>	
					<td align="left"><textarea name="headline" style="width: 200px; height: 75px;">{$headline}</textarea></td>
				</tr>
				<tr>
					<td align="left" colspan="2">
						<input type="button" value="{$lang.buttons.save_1}" class="btn_small" onclick="javascript: document.step_8.action='{$form.next_link}'; document.step_8.submit();">
					</td>
				</tr>
			</table>
			</td>
...
<TR><TD>
	<!--//UPLOAD VIDEO-->
	<form method="POST" name="upload_video" id="upload_video" enctype="multipart/form-data">
	<input name="choise" type="hidden" value="{$choise}">
	<input name="id_ad" type="hidden" value="{$id_ad}">
	<table cellpadding="0" cellspacing="0" border="0" width="100%">
		<tr valign="top">
		...
		<tr valign="top">
			<td class="subheader"><b>{$lang.content.page_header_step_upload_video}</b></td>
		</tr>
	</table>
	<table cellpadding="0" cellspacing="0" border="0" width="100%">
		<tr>
			...
			<table cellpadding="5" cellspacing="0" border="0" width="100%">
					...
					<tr>
						...
						<td valign="top" style="padding-left: 10px;">
							...
							{if $data.use_video_approve}
							{$lang.content.use_video_approve}
							{/if}
						</td>
					</tr>
					<tr>
						<td colspan="2">
							<table cellpadding="0" cellspacing="0">
								<tr>
									<td><input type="button" class="btn_small" value="{$lang.buttons.upload}" onclick=" document.getElementById('result_video').innerHTML='<img src=\'{$site_root}{$template_root}{$template_images_root}/indicator.gif\' style=\'height:16px;\'>'; jsLoad(this.form.video, 'result_video','{$id_ad}', 'v', document.getElementById('upload_comment').value, 'user_upload_video', '{$choise}', 'video', 'file_text_video', 'file_img_video');"></td>
									<td style="padding-left:10px;">
...
<TR><TD valign="top">
	<table cellpadding="0" cellspacing="0" border="0" width="100%">
		<tr valign="top">
			<td class="header"><b>{$lang.headers.rental_ad_edit}&nbsp;|&nbsp;
			{if $profile.type eq '1'}{$lang.content.need_room_big}
			{elseif $profile.type eq '2'}{$lang.content.have_room_big}
			{elseif $profile.type eq '3'}{$lang.content.buy_realty_big}
			{elseif $profile.type eq '4'}{$lang.content.sell_realty_big}
			{/if}</b></td>
		</tr>
		<tr valign="top">
			<td class="subheader"><b>{$lang.content.search_position}</b></td>
		</tr>
	</table>
	{strip}
	<table cellpadding="5" cellspacing="0" border="0" style="margin-left: 10px;">	
		...
		<tr>
			<td>{$lang.content.mark_ad}:&nbsp;&nbsp;
				<a href="{$server}{$site_root}/services.php?sel=top_search_ad&type=rent&id_ad={$profile.id}">{$lang.content.link_text_1}</a>&nbsp;&nbsp;|&nbsp;&nbsp;
				<a href="{$server}{$site_root}/services.php?sel=slideshow_ad&type=rent&id_ad={$profile.id}">{$lang.content.link_text_2}</a>&nbsp;&nbsp;|&nbsp;&nbsp;
				<a href="{$server}{$site_root}/services.php?sel=feature_ad&type=rent&id_ad={$profile.id}">{$lang.content.link_text_3}</a>
			</td>
		</tr>			
		{/if}
		<!--<tr>
	    ...
	    if (doc){
	    	switch (act){
	    		case "view":
	    			...
	    			url = "rentals.php?sel=" + step + "&ajax=1&id_ad=" + id_ad + "&choise=" + choise;
	    		break;
	    		case "file_up":
	    		case "file_down":
	    		case "upload_del":
	    		...
	    		case "upload_activate":
	    			url = "rentals.php?sel=" + act + "&ajax=1&id_ad=" + id_ad + "&file_id=" + upload_id + "&file_type=" + upload_type + "&choise=" + choise;
	    		break;
	    		case "edit_comment":
	    			...
	    			url = "rentals.php?sel=" + act + "&ajax=1&id_ad=" + id_ad + "&file_id=" + upload_id + "&file_type=" + upload_type + "&choise=" + choise + "&edit_upload_comment=" + new_pattern;
	    		break;
	    	}	    	
...
{if $form.sel eq 'step_1' && !($private_person_ads_limit && $user_ads_cnt >= $private_person_ads_limit)}
	{literal}
	<script language="javascript">
	function Show(pos){
		
		document.getElementById('zip_code_div_1').style.display = 'inline';
		document.getElementById('street_div_1').style.display = 'inline';
		document.getElementById('adress_div_1').style.display = 'inline';
	...
	var pay_sell_lease='{/literal}{$pay_sell_lease}{literal}';	
	
	
	if (choise != 0) {
		if ((choise == 2 || choise == 4) && (pay_sell_lease != 0))	{		
			choise = 1;
		}
		if (choise == 1 || choise == 3) {			
				Hide('1');			
