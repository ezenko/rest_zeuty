...
<tr valign="top">
	...
	<td class="main">
		...
		{/if}	
			<tr>
				<td class="header"><b>{$lang.headers.rental_ad_edit}</b></td>
			</tr>
		</table>		
...
{*		<hr>
		...
		<tr>
			...
			<td colspan="2">{$lang.content.list_text_1}:&nbsp;<b>{if $profile.account.user_type eq 1}{$lang.content.user_type_1}{else}{$lang.content.user_type_2}{/if}</b></td>
		</tr>		
		{if	$private_person_ads_limit && ($user_ads_cnt > $private_person_ads_limit)}
		<tr>
			<td width="15" height="27">&nbsp;</td>
			<td colspan="2">{$lang.content.your_ads_cnt} <b>{$user_ads_cnt}</b> {$lang.content.listings}.<br>{$lang.content.private_ads_limit_intro} - <b>{$private_person_ads_limit}</b> {$lang.content.private_ads_limit_message}<br><a href="{$server}{$site_root}/account.php">{$lang.content.change_status}</a></td>
		</tr>		
		{/if}
		</table>		
*}		
		...
		<tr>
			...
			<td width="100" align="right" valign="top" style="padding-right: 20px;"><a href="{$server}{$site_root}/rentals.php?sel=listing_position&id_ad={$profile.id}">{$lang.buttons.change}</a></td>
		</tr>
		{/if}
		...
		<tr>		
			<td>&nbsp;</td>
			<td colspan="2"  style="padding-right: 20px; padding-left:3px;">
			<table cellpadding="0" cellspacing="0" width="100%" align="right" >
				...
				<tr>
					...
					<td style="padding-top:5px;" >		
						<b>{$lang.content.reserved_from}</b> &nbsp;{$item.start}&nbsp; <b>{$lang.content.reserved_to}</b> &nbsp;{$item.end}			
					</td>		
					
						
					<td width="100" align="right" style="padding-top:4px;" >
					<a href="{$file_name}?sel=edit_event&amp;id_ad={$profile.id}&amp;id_event={$item.id}">{$lang.buttons.change}</a>&nbsp;
					<a href="{$file_name}?sel=delete_event&amp;id_ad={$profile.id}&amp;id_event={$item.id}">{$lang.buttons.delete}</a>
					</td>	
				</tr>
				{/foreach}		
			</table>	
			...
			<td>
				...
				<!-- have/sell realty -->
					{if $profile.min_payment>0}
						...
						{section name=b loop=$profile.period}
							<tr>
								<td width="200"><b>{$profile.period[b].name}:&nbsp;</b></td>
						...
						<tr>
							<td width="200"><b>{$lang.content.deposit}:&nbsp;</b></td>
							<td>{$profile.min_deposit_show}</td>
						</tr>
						{/if}
						{if $profile.movedate}
						<tr>
							<td width="200"><b>{$lang.content.available_date}:&nbsp;</b></td>
							<td>
							<table cellpadding="0" cellspacing="0" border="0" width="100%">
								<tr>
									<td>{$profile.movedate}</td>																
									<td>{if $profile.type eq '2'}<a href="{$file_name}?sel=calendar&id_ad={$profile.id}">{$lang.content.view_calendar}</a>{/if}</td>
								</tr>
							</table>
							</td>
						</tr>
						...
						{section name=b loop=$profile.realty_type}
							<tr>
								<td width="200"><b>{$profile.realty_type[b].name}:&nbsp;</b></td>
						...
						<tr>
							<td width="200"><b>{$lang.content.year_build}:&nbsp;</b></td>
						...
						{section name=b loop=$profile.description}
							<tr>
								<td width="200"><b>{$profile.description[b].name}:&nbsp;</b></td>
						...
						<tr>
							<td width="200"><b>{$lang.content.live_square}:&nbsp;</b></td>
						...
						<tr>
							<td width="200"><b>{$lang.content.property_floor_number}:&nbsp;</b></td>
						...
						<tr>
							<td width="200"><b>{$lang.content.subway_min}:&nbsp;</b></td>
						...
						{section name=b loop=$profile.info}
							<tr>
								<td width="200"><b>{$profile.info[b].name}:&nbsp;</b></td>
				...
				<!-- need/buy realty -->
					{if $profile.max_payment>0}
						...
						<tr>
							<td width="200"><b>{if $profile.type eq 1}{$lang.content.month_payment}{else}{$lang.content.price}{/if}:&nbsp;</b></td>
						...
						{if $profile.movedate}
						<tr>
							<td ><b>{$lang.content.move_date}:&nbsp;</b></td>
							<td>{$profile.movedate}</td>
						</tr>
						{/if}
						<!-- realty type -->
				...
				<table cellpadding="3" cellspacing="0" border="0" class="table_top">
					<tr><td>
							...
							<tr>
								...
								<td style="padding-left: 6px;"><font class="error">{if $profile.plan_photo_id}{$lang.content.no_approved}{else}{$lang.content.no_photos}{/if}</font></td>
							</tr>
							</table>
							{/if}
					</td></tr>
	...
	{/if}
				...
				<table cellpadding="3" cellspacing="0" border="0" class="table_top">
					<tr><td>
							...
							<tr>
								...
								<td style="padding-left: 6px;"><font class="error">{if $profile.photo_id}{$lang.content.no_approved}{else}{$lang.content.no_photos}{/if}</font></td>
							</tr>
							</table>
							{/if}
					</td></tr>
		...
		<tr>
			...
			<td style="padding-left: 3px;">
				{if $profile.video_id && $profile.video_approve_total}
				<b>{$lang.content.ad_text_video}</b>:&nbsp;
					<table cellpadding="3" cellspacing="3" border="0">
					{assign var=count value=0}
						{section name=ph loop=$profile.video_id}
						{if $count is div by 3}<tr>{/if}
						{if $profile.video_admin_approve[ph] == 1}
						{assign var=count value=$count+1}
						<td width="{$thumb_width+10}" valign="top">
							<table cellpadding="0" cellspacing="0" border="0">
							<tr>
				...
				{elseif $profile.video_id}
					<table cellpadding="0" cellspacing="0">
					<tr>
						<td width="200"><b>{$lang.content.ad_text_video}:</b></td>
						<td style="padding-left: 6px;"><font class="error">{$lang.content.no_approved}</font></td>
					</tr>	
					</table>					
				{else}
					<table cellpadding="0" cellspacing="0">
		...
		<tr>
			...
			<td>
				<table cellpadding="3" cellspacing="0" border="0" class="table_top">
						<tr>
							<td colspan="3">{if $profile.account.user_type eq 1 || $profile.account.user_type eq 3}{$lang.content.account_about_me}{else}{$lang.content.account_about_us}{/if}</td>
						</tr>
						<tr>
							<td width="200"><b>{$lang.content.fname}:</b></td>
							<td colspan="2">{$profile.account.fname}</td>
						</tr>
						<tr>
							<td width="200"><b>{$lang.content.sname}:</b></td>
							<td colspan="2">{$profile.account.sname}</td>
						</tr>
						...
						<tr>
							...
							<td colspan="2">{$profile.account.email}</td>
						</tr>
						{if $profile.account.phone}
						<tr>
							<td width="200"><b>{$lang.content.phone}:</b></td>
							<td colspan="2">{$profile.account.phone}</td>
						</tr>
						{/if}
						{if $profile.account.user_type eq 2}
						{* realtor *}
							<tr>
								<td width="100"><b>{$lang.content.company_name}:</b></td>
								<td colspan="2">{$profile.account.company_name}</td>
							</tr>
							{if $profile.account.company_url != ""}
							<tr>
								<td width="200"><b>{$lang.content.company_url}:</b></td>
								<td colspan="2">{$profile.account.company_url}</td>
							</tr>
							{/if}
							<tr>
							...
							<tr>
								...
								<td colspan="2">{$profile.account.country_name}{if $profile.account.region_name}, {$profile.account.region_name}{/if}{if $profile.account.city_name}, {$profile.account.city_name}{/if}{if $profile.account.address}, {$profile.account.address}{/if}</td>
							</tr>
							{/if}
							
							{if $profile.account.postal_code != ""}
							<tr>
								<td width="200"><b>{$lang.content.zipcode}:</b></td>
								<td colspan="2">{$profile.account.postal_code}</td>
							</tr>
							{/if}
						...
						{* private person *}
							<tr>
								...
								<td colspan="2">{$profile.account.birth_month}.{$profile.account.birth_day}.{$profile.account.birth_year}</td>
							</tr>
							{section name=b loop=$profile.gender}
							<tr>
								<td width="200"><b>{$profile.gender[b].name}:&nbsp;</b></td>
								<td colspan="2">{section name=c loop=$profile.gender[b].fields}{$profile.gender[b].fields[c]}{if !$smarty.section.c.last},&nbsp;{/if}{/section}
								</td>
							</tr>
							{/section}
							...
							<tr>
								...
								<td colspan="2">{section name=c loop=$profile.people[b].fields}{$profile.people[b].fields[c]}{if !$smarty.section.c.last},&nbsp;{/if}{/section}
								</td>
							</tr>
							{/section}
							...
							<tr>
								...
								<td colspan="2">{section name=c loop=$profile.language[b].fields}{$profile.language[b].fields[c]}{if !$smarty.section.c.last},&nbsp;{/if}{/section}
								</td>
							</tr>
							{/section}
			...
			<td width="100" align="right" valign="top" style="padding-right: 20px;"><a href="account.php?redirect={$profile.id}">{$lang.buttons.change}</a></td>
		</tr>
		<!-- /account info -->
		<tr><td colspan="3"><hr></td></tr>
		{if $no_ad ne 1}
		<tr>
			...
			<td>
				<table cellpadding="3" cellspacing="0" border="0" class="table_top">
					<tr>
						<td width="200"><b>{$lang.content.comments}:&nbsp;</b></td>
		...
		{if $get_bonus && !$mhi_services}
			...
			<tr>
				<td width="13">&nbsp;</td>
				<td colspan="2">{$lang.default_select.indicator_text_2}{$percent}%.</td>
			</tr>
			<tr>
				<td width="13">&nbsp;</td>
				<td colspan="2">{$lang.default_select.indicator_text_3}{$cur_symbol}{$can_recieve}&nbsp;{$lang.default_select.indicator_text_4}.</td>
			</tr>
			{if $can_recieve ne '0'}
				<tr>
					<td width="13">&nbsp;</td>
					<td colspan="2">
						<a href="{$file_name}?sel=get_bonus&id_ad={$profile.id}">{$lang.content.get_bonus}</a>
					</td>
				</tr>
			{/if}
		{/if}
