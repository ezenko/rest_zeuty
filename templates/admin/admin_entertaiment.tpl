{include file="$admingentemplates/admin_top.tpl"}
	<table cellpadding="0" cellspacing="0" border="0" width="100%">
		<tr>
			<td class="header" align="left">{$lang.menu.content_management} <font class="subheader">| {$lang.menu.sections_management} | {$lang.menu.entertaiment}</font></td>
		</tr>
		<tr>
		...
		<tr>
			...
			{if $sel == "main"}
				...
				{if $entertaiments}
					...
					{foreach from=$entertaiments item=page name=bottom_sections}
					<tr>
			...
			{elseif $sel == "add_section" || $sel == "edit_section"}
					...
					<td>
                        ...
                        {foreach from=$rest item=r}
                          {if $r.id eq 3}
                    	    ...
                    	    <option value="{$opt.value}" {if $type_id eq $opt.value} selected {/if}>{$opt.name}</option>
                    	    {/foreach}
                    	  {/if}
                        {/foreach}
                        </select>
                ...
                <tr>
					...
					<td id="country_div">
						<select name="country" id="country" onchange="javascript: {literal} SelectRegion3('ip', this.value, document.getElementById('region'), document.getElementById('city'),'{/literal}{$lang.content.ip_load_region}{literal}', '{/literal}{$lang.content.ip_city}{literal}'); {/literal}" class="location">
				...
				<tr>
					...
					<td id="region_div">
						<select name="region"  id="region" onchange="javascript: {literal} SelectCity3('ip', this.value, document.getElementById('city'), '{/literal}{$lang.content.ip_load_city}{literal}');{/literal}" class="location">
						<option value="">{$lang.content.ip_region}</option>
							{foreach item=item from=$region}
						<option value="{$item.id}"  {if $region_id eq $item.id} selected {/if}>{$item.name}</option>
							{/foreach}
						</select>
					</td>														
				</tr>
