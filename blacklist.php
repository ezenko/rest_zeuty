...
* @see viewprofile.php - add to blacklist
*
* @package RealEstate
* @subpackage User Mode
* @copyright Pilot Group <http://www.pilotgroup.net/>
* @author $Author: irina $
* @version $Revision: 1.2 $ $Date: 2008/10/13 07:57:24 $
**/
...
if($user[4]==1 && !(isset($_REQUEST["view_from_admin"]) && $_REQUEST["view_from_admin"] == 1)){
	...
	switch ($sel){
		case "del": 	DelFromBlackList(); break;
		default: 		BlacklistTable();
	}
}
...
function BlackListTable(){
	...
	IndexHomePage('blacklist', 'homepage');

	if($user[3] != 1){		                //homepage menu if user registered
		CreateMenu('homepage_top_menu');
		CreateMenu('homepage_user_menu');
	} else {			//index menu if user not registered
	...
	$smarty->assign("submenu", "blackthemrent");

	$page = (isset($_REQUEST["page"]) && intval($_REQUEST["page"]) > 0) ? intval($_REQUEST["page"]) : 1;

	//all friends' id of our user
	$strSQL = "SELECT id_enemy FROM ".BLACKLIST_TABLE." WHERE id_user='".$user[0]."' ORDER BY id_enemy";
	$rs = $dbconn->Execute($strSQL);
	$i = 0;
	while(!$rs->EOF){
		$row = $rs->GetRowAssoc(false);
		$id_arr[$i] = $row["id_enemy"];
		$rs->MoveNext();
		$i++;
	}
	...
	if ($num_records>0){
		...
		$strSQL = "	SELECT DISTINCT a.id, a.fname, a.date_birthday, DATE_FORMAT(a.date_last_seen,'".$config["date_format"]."')  as date_last_login, e.id_user as session, DATE_FORMAT(bt.datenow, '".$config["date_format"]."') as date_blacklisted, a.user_type
					FROM ".USERS_TABLE." a
					LEFT JOIN ".ACTIVE_SESSIONS_TABLE." e on a.id=e.id_user
					LEFT JOIN ".BLACKLIST_TABLE." bt ON (a.id=bt.id_enemy AND bt.id_user='".$user[0]."')
		...
		while(!$rs->EOF){
			...
			$users_list[$i]["date_added"] = $row["date_blacklisted"];

			$suffix = "&id=".$users_list[$i]["id"];

			$users_list[$i]["mail_link"] = "./mailbox.php?sel=fs".$suffix;
...
function DelFromBlackList() {
	...
	$id_enemy = intval($_GET["id_user"]);
	$strSQL = "SELECT id FROM ".BLACKLIST_TABLE." WHERE id_user='".$user[0]."' AND id_enemy='".$id_enemy."' ";
	$rs = $dbconn->Execute($strSQL);
	if ($rs->fields[0]>0) {
		$dbconn->Execute("DELETE FROM ".BLACKLIST_TABLE." WHERE id='".$rs->fields[0]."' ");
	}
	BlackListTable();
	exit;
}
